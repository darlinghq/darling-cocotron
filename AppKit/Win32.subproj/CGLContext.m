...
/* There is essentially only two ways to implement a CGLContext on Windows.
   ...
   test. You can't use the front buffer in a double buffered window for the
   same reasons.
...
CGL_EXPORT CGLError CGLCreateContext(CGLPixelFormatObj pixelFormat,
 ...
 context to be current, save it and restore it. Some applications may create a
 context while using another, so we don't want to improperly switch to
 another context during creating */
...
HPBUFFERARB
CGLGetPBUFFER(CGLPBufferObj pbuffer) {
