...
HANDLE
openThemeData(HWND window, LPCWSTR classList) {
...
static BOOL drawThemeBackground(HTHEME theme, HDC dc, int partId, int stateId,
    ...
    if (function(theme, dc, partId, stateId, rect, clip) != S_OK) {
        NSLog(@"DrawThemeBackground(%x,%x,%d,%d,{%d %d %d %d}) failed", theme,
              dc, partId, stateId, rect->top, rect->left, rect->bottom,
              rect->right);
        return NO;
    }
    return YES;
...
- (BOOL) marginsOfPartId: (int) partId
    ...
    if ((theme = [[deviceContext windowDeviceContext] theme: uxthClassId]) !=
        NULL) {
        MARGINS margins;

        if (getThemeMargins(theme, [deviceContext dc], partId, stateId,
                            /*TMT_CONTENTMARGINS*/ 3602, NULL, &margins)) {
...
- (BOOL) valueForPartId: (int) partId
                stateId: (int) stateId
                 propId: (int) propId
            uxthClassId: (int) uxthClassId
                  value: (int *) result {
...
- (BOOL) sizeOfPartId: (int) partId
    ...
    if ((theme = [[deviceContext windowDeviceContext] theme: uxthClassId]) !=
        ...
        if (getThemePartSize(theme, [deviceContext dc], partId, stateId, NULL,
                             TS_DRAW, &size)) {
...
- (BOOL) drawPartId: (int) partId
    ...
    if ((theme = [[deviceContext windowDeviceContext] theme: uxthClassId]) !=
        ...
        drawThemeBackground(theme, [deviceContext dc], partId, stateId, &tlbr,
                            NULL);
...
- (NSSize) menuItemSeparatorSize {
    ...
    if (![self sizeOfPartId: /*MENU_POPUPSEPARATOR*/ 15
                    stateId: 0
                uxthClassId: uxthMENU
                       size: &result]) {
        return [super menuItemSeparatorSize];
    } else if (result.height == 0.0f) {
        result.height = 6.0f; // Why I don't know, but this doesn't return a
                              // non-zero value on Win7.
    }
    return result;
}
...
- (Margins) menuItemTextMargins {
    Margins result;
    if (![self marginsOfPartId: /*MENU_POPUPITEM*/ 14
                       stateId: 0
                   uxthClassId: uxthMENU
                       margins: &result]) {
...
- (NSSize) menuItemBranchArrowSize {
    ...
    if (![self sizeOfPartId: /*MENU_POPUPSUBMENU*/ 16
                    stateId: 0
                uxthClassId: uxthMENU
                       size: &result]) {
...
- (NSSize) menuItemCheckMarkSize {
    ...
    if (![self sizeOfPartId: /*MENU_POPUPCHECK*/ 11
                    stateId: 0
                uxthClassId: uxthMENU
                       size: &result]) {
...
- (Margins) menuItemGutterMargins {
    ...
    if (![self marginsOfPartId: /*MENU_POPUPCHECK*/ 11
                       stateId: 0
                   uxthClassId: uxthMENU
                       margins: &result]) {
...
- (CGFloat) menuBarHeight {
    int result = 0;
    if (![self valueForPartId: /*MENU_BARBACKGROUND*/ 7
                      stateId: 0
                       propId: /*TMT_MENUBARHEIGHT*/ 1209
                  uxthClassId: uxthMENU
                        value: &result]) {
...
- (void) drawPushButtonNormalInRect: (NSRect) rect defaulted: (BOOL) defaulted {
    if (![self drawButtonPartId: BP_PUSHBUTTON
                        stateId: defaulted ? PBS_DEFAULTED : PBS_NORMAL
                         inRect: rect]) {
...
- (BOOL) getPartId: (int *) partId
    ...
    if ([[image name] isEqual: @"NSSwitch"]) {
        ...
        *partId = BP_CHECKBOX;
        *stateId = mixed ? (enabled ? CBS_MIXEDNORMAL : CBS_MIXEDDISABLED)
                         : (enabled ? CBS_CHECKEDNORMAL : CBS_CHECKEDDISABLED);
        valid = YES;
...
- (NSRect) drawProgressIndicatorBackground: (NSRect) rect
    ...
    return [super drawProgressIndicatorBackground: rect
                                         clipRect: clipRect
                                          bezeled: bezeled];
}
...
- (void) drawScrollerButtonInRect: (NSRect) rect
    ...
    if (vertical) {
        if (upOrLeft) {
            stateId = enabled ? (pressed ? ABS_UPPRESSED : ABS_UPNORMAL)
                              : ABS_UPDISABLED;
        } else {
            stateId = enabled ? (pressed ? ABS_DOWNPRESSED : ABS_DOWNNORMAL)
                              : ABS_DOWNDISABLED;
        }
    } else {
        if (upOrLeft) {
            stateId = enabled ? (pressed ? ABS_LEFTPRESSED : ABS_LEFTNORMAL)
                              : ABS_LEFTDISABLED;
        } else {
            stateId = enabled ? (pressed ? ABS_RIGHTPRESSED : ABS_RIGHTNORMAL)
                              : ABS_RIGHTDISABLED;
        }
    }
