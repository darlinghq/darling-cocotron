...
// That's usually a bad idea if the UI can't automatically adapt to a random
// control sizes
...
- (Win32Font *) createGDIFontSelectedInDC: (HDC) dc {
    ...
    Win32Font *result = [[Win32Font alloc] initWithName: (NSString *) _name
                                                 height: -height
                                              antialias: NO];
...
- (Win32Font *) selectFontInDefaultDC {
    ...
    if (_winFont == nil) {
        _winFont = [(O2Font_gdi *) _font createGDIFontSelectedInDC: _dc
                                                         pointSize: _size];
    }
...
- (void) loadGlyphRangeTable {
    ...
    // GetGlyphIndicesW is around twice as fast as GetCharacterPlacementW, but
    // only available on Win2k/XP
    if (getGlyphIndices != NULL) {
        ...
        if (GetCharacterPlacementW(_dc, characters, range.length, 0, &results,
                                   0) == 0) {
...
- (void) fetchGlyphKerning {
    ...
    // We need to use the font at Em size to get precise (or just correct for
    // font) metrics
    LOGFONT logFont;
    ...
    for (i = 0; i < numberOfPairs; i++) {
        ...
        if (current == NSNullGlyph) {
            ; // NSLog(@"unable to generate kern pair 0x%04X 0x%04X
              // %f",previousCharacter,currentCharacter,xoffset);
        } else {
...
- (void) fetchGlyphInfo {
    _glyphInfoSet->info = NSZoneCalloc([self zone], sizeof(CGGlyphMetrics),
                                       _glyphInfoSet->numberOfGlyphs);
...
- (void) fetchMetrics {
 ...
 A font with height of negative otmEMSquare will have precise metrics  */
    ...
    if (![(NSString *) _name isEqualToString: @"Marlett"] &&
        ![(NSString *) _name isEqualToString: @"Segoe UI"] &&
        ![(NSString *) _name isEqualToString: @"Tahoma"]) {
        _useMacMetrics = YES;
    }
        ...
        if (_glyphInfoSet->info != NULL) {
            ...
            for (i = 0; i < _glyphInfoSet->numberOfGlyphs; i++) {
                if (_glyphInfoSet->info[i].kerningOffsets != NULL) {
                    NSZoneFree([self zone],
                               _glyphInfoSet -> info[i].kerningOffsets);
                }
            }
    ...
    *isNominalp = YES;
    ...
    if (previous == CGNullGlyph) {
        ...
        if (previousInfo != NULL) {
            CGFloat delta = previousInfo->advanceA + previousInfo->advanceB +
                            previousInfo->advanceC;
            if (_useMacMetrics == NO) {
                delta = roundf(delta);
            }
            result.x += delta;
...
static FIXED fxDiv2(FIXED fxVal1, FIXED fxVal2) {
    // Note: the "volatile" is there to prevent some wrong result because of
    // some compiler "optimisations"
    int32_t l = (*((int32_t volatile *) &(fxVal1)) +
                 *((int32_t volatile *) &(fxVal2))) /
                2;
...
static UINT AppendQuadBSplineToBezier(POINTFX start, LPTTPOLYCURVE lpCurve,
    ...
    for (i = 0; i < lpCurve->cpfx;) {
        ...
        O2PathAddCurveToPoint(path, NULL, bezier[1].x, bezier[1].y, bezier[2].x,
                              bezier[2].y, bezier[3].x, bezier[3].y);
...
// fixed-to-int conversions replaced with fixed-to-float
static void ConvertTTPolygonToPath(LPTTPOLYGONHEADER lpHeader, DWORD size,
    ...
    while ((DWORD) lpHeader < (DWORD)(((LPSTR) lpStart) + size)) {
        if (lpHeader->dwType == TT_POLYGON_TYPE) {
            ...
            while ((DWORD) lpCurve <
                   (DWORD)(((LPSTR) lpHeader) + lpHeader->cb)) {
                //**********************************************
                // Format assumption:
                ...
                if (lpCurve->wType == TT_PRIM_LINE) {
                    ...
                    // Decode each Quadratic B-Spline segment, convert to
                    // bezier, and append to the Bezier segments
...
- (O2Path *) createPathForGlyph: (CGGlyph) glyph
 ...
 A font with height of negative otmEMSquare will have precise metrics  */
    ...
    int outlineSize = GetGlyphOutline(_dc, glyph, GGO_NATIVE | GGO_GLYPH_INDEX,
    ...
    if (outlineSize == GDI_ERROR) {
        ...
        if (GetGlyphOutline(_dc, glyph, GGO_NATIVE | GGO_GLYPH_INDEX,
                            &glyphMetrics, outlineSize, outline,
                            &mat2) != GDI_ERROR) {
