...
#if STBI_SIMD
                stbi_idct_installed(z->img_comp[n].data + z->img_comp[n].w2 * j * 8 + i * 8, z->img_comp[n].w2, data,
...
#if STBI_SIMD
                            stbi_idct_installed(z->img_comp[n].data + z->img_comp[n].w2 * y2 + x2, z->img_comp[n].w2, data,
                                                z->dequant2[z->img_comp[n].tq]);
#else
...
static unsigned char* stbi_jpeg_load_from_memory(jpeg* j, stbi_uc const* buffer, int len, int* x, int* y, int* comp,
        int req_comp) {
