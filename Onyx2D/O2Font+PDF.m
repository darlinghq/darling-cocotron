...
uint8_t O2PDFMacRomanPositionOfGlyphName(NSString *name) {
    ...
    if (map == nil) {
        ...
        int i;
        for (i = 0; i < 256; i++) {
            [mutmap setObject: [NSNumber numberWithInt: i]
                       forKey: O2MacRomanGlyphNames[i]];
        }
        map = mutmap;
    }
...
- (O2PDFDictionary *) _pdfFontDescriptorWithSize: (CGFloat) size {
    ...
    [result setObjectForKey: "FontBBox"
                      value: [O2PDFArray pdfArrayWithNumbers: bbox count: 4]];
    [result setIntegerForKey: "ItalicAngle"
                       value: O2FontGetItalicAngle(self) / unitsPerEm * size];
    [result setIntegerForKey: "Ascent"
                       value: O2FontGetAscent(self) / unitsPerEm * size];
    [result setIntegerForKey: "Descent"
                       value: O2FontGetDescent(self) / unitsPerEm * size];
    [result setIntegerForKey: "CapHeight"
                       value: O2FontGetCapHeight(self) / unitsPerEm * size];
