...
BOOL seekToTable(O2TTFDecoderRef self, uint32_t seekToTag) {
    ...
    dump(self, @"unable to find tag %c%c%c%c", seekToTag >> 24,
         (seekToTag >> 16) & 0xFF, (seekToTag >> 8) & 0xFF, seekToTag & 0xFF);
    return NO;
}
static struct {
...
NSMapTable *O2TTFDecoderGetPostScriptNameMapTable(O2TTFDecoderRef self,
    ...
    case 0x00020000:;
        ...
        for (i = 0; i < maxIndex; i++) {
            ...
            names[i] = [[NSString alloc] initWithBytes: buffer
                                                length: count
        ...
        for (i = 0; i < numberOfGlyphs; i++) {
            ...
            if (nameIndex <= 257) {
                NSMapInsert(result, MacintoshNameMapping[nameIndex].name,
                            (void *) i);
            } else {
                nameIndex -= 258;
...
void O2TTFDecoderGetNameTable(O2TTFDecoderRef self) {
    ...
    for (i = 0; i < count; i++) {
        ...
        NSLog(@"platformID=%d,languageId=%d,nameID=%d", platformID, languageID,
              nameID);

        NSLog(@"position=%ld,stringOffset=%d,offset=%d", self->_position,
              stringOffset, offset);

        NSString *string =
            [NSString stringWithCString: (const char *) self->_bytes + location
                                 length: length];

        NSLog(@"platformID=%d,languageID=%d,string=%@", platformID, languageID,
              string);
    }
}
