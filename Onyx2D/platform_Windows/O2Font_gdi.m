...
static int CALLBACK EnumFontFromFamilyCallBack(const EXTLOGFONTW *logFont,
    ...
    // CreateFontIndirectW is returning the wrong font for some faces like
    // "Minya Nouvelle Italic" (the Regular version is returned instead)
    HFONT font =
        CreateFontW(0, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                    OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                    DEFAULT_PITCH | FF_DONTCARE, logFont->elfFullName);

    if (font) {
        // Ignore fonts for which we can't get a full name
        if (logFont->elfFullName == NULL) {
            if (logFont->elfLogFont.lfFaceName) {
                NSLog(@"NULL full name for face name %S",
                      logFont->elfLogFont.lfFaceName);
            } else {
        ...
        NSString *winName =
            [NSString stringWithFormat: @"%S", logFont->elfFullName];
        if (winName.length == 0) {
            if (logFont->elfLogFont.lfFaceName) {
                NSLog(@"Empty full name for face name %S",
                      logFont->elfLogFont.lfFaceName);
            } else {
        ...
        DWORD bufferSize =
            GetFontData(dc, CFSwapInt32HostToBig('name'), 0, NULL, 0);
        if (bufferSize >= 6 && bufferSize != GDI_ERROR) {
            uint8_t buffer[bufferSize];
            bzero(buffer, 6);
            if (GetFontData(dc, CFSwapInt32HostToBig('name'), 0, buffer,
                            bufferSize) != GDI_ERROR) {
                ...
                uint16_t stringsOffset =
                    CFSwapInt16BigToHost(header->stringOffset);
                if (bufferSize > stringsOffset) {
                    uint8_t *strings = buffer + stringsOffset;
                    name_records_t *nameRecords =
                        (name_records_t *) (buffer +
                                            sizeof(name_table_header_t));
                    // Parse all of the name records until we find a US full
                    // name there so we can have both the US name and the
                    // localized name in the font name -> PS name table
                    NSString *fontNameUS = nil;
                    for (int i = 0; i < count; i++) {
                        uint16_t platformID =
                            CFSwapInt16BigToHost(nameRecords[i].platformID);
                        uint16_t platformSpecificID = CFSwapInt16BigToHost(
                            nameRecords[i].platformSpecificID);
                        uint16_t languageID =
                            CFSwapInt16BigToHost(nameRecords[i].languageID);
                        uint16_t nameID =
                            CFSwapInt16BigToHost(nameRecords[i].nameID);
                        uint16_t length =
                            CFSwapInt16BigToHost(nameRecords[i].length);
                        uint16_t offset =
                            CFSwapInt16BigToHost(nameRecords[i].offset);
                        // Full name, US language
                        if (nameID == 4 && languageID == 0x0409) {
                            // First check we get a string that's in our buffer,
                            // in case of some corrupted font or font entry
                            if (stringsOffset + offset + length <= bufferSize) {
                                // Check for Win+Unicode encoding or Mac+Roman
                                // encoding Other entries should be ignored (see
                                ...
                                if (platformID == 3 &&
                                    ...
                                    NSString *name = [NSString
                                        stringWithCString: strings + offset
                                                   length: length];
                                    if ([name length]) {
                                        if (![winName isEqualToString: name]) {
                                            fontNameUS = name;
                    ...
                    // Parse all of the name records until we find some PS name
                    // there
                    for (int i = 0; i < count; i++) {
                        uint16_t platformID =
                            CFSwapInt16BigToHost(nameRecords[i].platformID);
                            ...
                            if (stringsOffset + offset + length <= bufferSize) {
                                ...
                                if (platformID == 3 &&
                                    ...
                                    NSString *name = [[[NSString alloc]
                                        ...
                                        autorelease];
                                    if ([name length]) {
                                        // It seems some users have some
                                        // corrupted font table - NEVER replace
                                        // Arial fonts with something which
                                        // isn't some kind of arial
                                        if (!([name hasPrefix: @"Arial"] &&
                                              [winName rangeOfString: @"Arial"]
                                                      .location ==
                                                  NSNotFound)) {
                                            [sPSToWin32Table setObject: winName
                                                                forKey: name];
                                        } else {
                                            NSLog(
                                                @"Skipping matching %@ with %@",
                                                name, winName);
                                        }
                                        [sWin32ToPSTable setObject: name
                                                            forKey: winName];
                                        if (fontNameUS) {
                                            // Add the US font name -> PS
                                            // mapping
                                            [sWin32ToPSTable
                                                setObject: name
                                                   forKey: fontNameUS];
...
+ (void) _buildNativePSmapping {
    ...
    EnumFontFamiliesExW(dc, &logFont, (FONTENUMPROCW) EnumFamiliesCallBackW,
                        (LPARAM) families, 0);
    for (NSString *familyName in families) {
        ...
        [familyName getCharacters: logFont.lfFaceName
                            range: NSMakeRange(0, length)];

        logFont.lfCharSet = DEFAULT_CHARSET;
        logFont.lfPitchAndFamily = 0;
        EnumFontFamiliesExW(dc, &logFont,
                            (FONTENUMPROCW) EnumFontFromFamilyCallBack,
                            (LPARAM) dc, 0);
    }
    ReleaseDC(NULL, dc);
...
+ (NSString *) nativeFontNameForPostscriptName: (NSString *) psName {
    ...
    @synchronized(self) {
        ...
        if (name == nil) {
            O2FontLog(@"failed to find native font for postscript name: %@",
                      psName);
            name = psName;
        }
    }
...
static HFONT Win32FontHandleWithName(NSString *name, int unitsPerEm) {
    const unichar *wideName =
        (const unichar *) [name cStringUsingEncoding: NSUnicodeStringEncoding];
    O2FontLog(@"Creating font with name: %@ unitsPerEm: %d", name, unitsPerEm);
    HFONT result =
        CreateFontW(-unitsPerEm, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                    DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                    DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, wideName);
    if (result == NULL) {
...
- initWithFontName: (NSString *) name {
    ...
    // P. 931 "Windows Graphics Programming" by Feng Yuan, 1st Ed. A font with
    // height of negative otmEMSquare will have precise metrics 2048 is a common
    // TrueType em square, so we try that first, and if the font is not
    // TrueType, maybe we'll get some decent metrics with a big size.
    _unitsPerEm = 2048;
    ...
    if (gdiMetrics.tmPitchAndFamily & TMPF_TRUETYPE) {
        ...
        if ((size = GetOutlineTextMetricsW(dc, size, ttMetrics)) != 0) {
            if (ttMetrics->otmEMSquare != _unitsPerEm) {
                // if our first try didn't have the typical em square, recreate
                // using the em square
                LOGFONT logFont;
            ...
            if (size != 0) {
                ...
                // Don't use the magic pointSize scaling formula on these font
                // (UI fonts)
                if ([_name isEqualToString: @"Marlett"] ||
                    [_name isEqualToString: @"Segoe UI"] ||
                    [_name isEqualToString: @"Tahoma"]) {
                    _useMacMetrics = NO;
                } else {
                    _useMacMetrics = YES;
                }
                ...
                _bbox.size.width = ttMetrics->otmrcFontBox.right -
                                   ttMetrics->otmrcFontBox.left;
                _bbox.size.height = ttMetrics->otmrcFontBox.top -
                                    ttMetrics->otmrcFontBox.bottom;
            }
        }
    }
...
- initWithDataProvider: (O2DataProviderRef) provider {
    ...
    _fontMemResource =
        AddFontMemResourceEx((PVOID) bytes, length, 0, &numberOfFonts);
    NSLog(@"FAILURE AddFontMemResourceEx bytes=%p length=%d "
          @"numberOfFonts=%d,result=%x",
          bytes, length, numberOfFonts, _fontMemResource);
