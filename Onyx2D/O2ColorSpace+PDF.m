...
+ (O2ColorSpaceRef) createColorSpaceFromPDFObject: (O2PDFObject *) object {
    ...
    } else if ([object checkForType: kO2PDFObjectTypeArray
                              value: &colorSpaceArray]) {
        const char *name;
        ...
        if (strcmp(name, "Indexed") == 0) {
            ...
            if ([colorSpaceArray getStringAtIndex: 3 value: &tableString]) {
                if ([tableString length] != tableSize) {
                    NSLog(@"lookup invalid size,string length=%d,tableSize=%d",
                          [tableString length], tableSize);
                    return nil;
                }
                ...
                if ([data length] != tableSize) {
                    NSLog(@"lookup invalid size,data length=%d,tableSize=%d",
            ...
            if (![colorSpaceArray getStreamAtIndex: 1 value: &stream]) {
                NSLog(@"second element of ICCBased color space array is not a "
                      @"stream");
                return nil;
            }
            dictionary = [stream dictionary];
            if (![dictionary getIntegerForKey: "N"
            ...
            if (![colorSpaceArray getNameAtIndex: 1 value: &name]) {
                O2PDFError(__FILE__, __LINE__,
                           @"No name available in color space %@", object);
                return nil;
            }

            if (![colorSpaceArray getObjectAtIndex: 2
                                             value: &alternateDictionary]) {
                O2PDFError(__FILE__, __LINE__,
                           @"No alternateSpace available in color space %@",
                           object);
                return nil;
            }
            ...
            if (![colorSpaceArray getObjectAtIndex: 3 value: &tintObject]) {
                O2PDFError(__FILE__, __LINE__,
                           @"No tintFunction available in color space %@",
                           object);
                return nil;
            }
            ...
            if (tintFunction == NULL) {
                O2PDFError(__FILE__, __LINE__,
                           @"unable to initialize tintFunction");
            }
            ...
            return O2ColorSpaceCreateDeviceN(cStringNames, alternateSpace,
                                             tintFunction);
            ...
            O2PDFError(__FILE__, __LINE__, @"does not handle color space %@",
                       object);
        }
    } else {
