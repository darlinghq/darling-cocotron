...
- (KGPDFArray *) _pdfWidths {
    ...
    // FIX, probably not entirely accurate, you can get precise widths out of
    // the TrueType data
    for (i = 0; i < 255; i++) {
...
- (KGPDFDictionary *) _pdfFontDescriptor {
    ...
    [result setObjectForKey: "FontBBox"
                      value: [KGPDFArray pdfArrayWithNumbers: bbox count: 4]];
...
- (KGPDFObject *) encodeReferenceWithContext: (KGPDFContext *) context {
    KGPDFObject *reference = [context referenceForFontWithName: [self name]
                                                          size: _size];
