...
- initWithPersistentStoreCoordinator:
    ...
    if ([data length] != 0) {
        if ((_document = [[NSXMLDocument alloc] initWithData: data
                                                     options: xmlOptions
                                                       error: NULL]) == nil) {
            [self dealloc];
            return nil;
        ...
        _document = [[NSXMLDocument alloc] initWithKind: NSXMLDocumentXMLKind
                                                options: xmlOptions];
...
- (NSAtomicStoreCacheNode *) loadEntityElement: (NSXMLElement *) entityElement
    ...
    for (NSXMLElement *attribute in attributeElements) {
        ...
        case NSDateAttributeType:
            ...
            //   objectValue=[NSCalendarDate
            //   dateWithNaturalLanguageString:stringValue];
            break;
    ...
    for (NSXMLElement *relationship in relationshipElements) {
        ...
        if (![description isToMany]) {
            if ([objectValue count] > 1) {
                NSLog(@"relationship description is not to many, but "
                      @"destination is %d",
                      [objectValue count]);
            }
...
- (BOOL) load: (NSError **) errorp {
    ...
    for (i = 0; i < count; i++) {
        ...
        NSAtomicStoreCacheNode *node = [self loadEntityElement: element
                                                         model: model];
    ...
    for (NSString *relationshipName in relationshipKeys) {
        ...
        if ([relationshipDescription isToMany]) {
            ...
            if ([cacheNodeSet count] == 0) {
                ...
                [node setValue: [cacheNodeSet anyObject]
